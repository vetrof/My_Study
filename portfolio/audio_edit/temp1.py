...
def _playsoundWin(sound, block=True):
    ...
    windll.winmm.mciSendStringW.argtypes = [wintypes.LPCWSTR, wintypes.LPWSTR,
                                            wintypes.UINT, wintypes.HANDLE]
    windll.winmm.mciGetErrorStringW.argtypes = [wintypes.DWORD,
    ...
    def winCommand(*command):
        ...
        errorCode = int(windll.winmm.mciSendStringW(command, buf, bufLen - 1,
                                                    0))  # use widestring version
        # of the function
        if errorCode:
            errorBuffer = create_unicode_buffer(bufLen)
            windll.winmm.mciGetErrorStringW(errorCode, errorBuffer,
                                            bufLen - 1)  # use widestring version
            # of the function
            exceptionMessage = (
...
def _handlePathOSX(sound):
    ...
    try:
        ...
        try:
            ...
            from urllib import \
                quote  # Try using the Python 2 import before giving up entirely...

        parts = sound.split('://', 1)
        return parts[0] + '://' + quote(parts[1].encode('utf-8')).replace(' ',
                                                                          '%20')
...
def _playsoundOSX(sound, block=True):
    ...
    try:
        ...
        logger.warning(
            "playsound could not find a copy of AppKit - falling back to"
            " using macOS's system copy.")
        sys.path.append(
...
if system == 'Windows':
    ...
    if sys.version_info[0] > 2:
        try:
            ...
            logger.warning(
                "playsound is relying on a python 2 subprocess. Please use "
                "`pip3 install PyObjC` if you want playsound "
    ...
    if __name__ != '__main__':  # Ensure we don't infinitely recurse trying to
        # get another python instance.
        try:
            import gi
            ...
            logger.warning(
                ...
                " Please use `pip install pygobject` if you want playsound to"
