...
def ranged(
    ...
    file.seek(start)
    while True:
        data_length = min(block_size,
                          end - start - consumed) if end else block_size
        if data_length <= 0:
            break
...
def open_file(request, video_pk: int) -> tuple:
    ...
    file_link = f'{settings.MEDIA_ROOT}/{_video.video_link}'

    file = open(f'{file_link}', 'rb')

    # file_size
    file_stat = os.stat(f'{file_link}')
